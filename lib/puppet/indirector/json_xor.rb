...
# The base class for "encrypted" JSON indirection terminus implementations.
#
...
class Puppet::Indirector::ROT13 < Puppet::Indirector::Terminus
  def find(request)
  ...
  def save(request)
    ...
    Puppet::Util.replace_file(filename, 0660) do |f|
      json_bytes = to_json(request.instance).bytes
      json_bytes.map! { |b| b ^ KEY }
      f.print json_bytes.pack('c*').force_encoding(Encoding::ASCII_8BIT)
    end
  ...
  def load_json_from_file(file, key)
    ...
    begin
      content = Puppet::FileSystem.read(file).bytes
      content.map! { |b| b ^ KEY }
      json = bytes.pack('c*').force_encoding(Encoding::ASCII_8BIT)
    rescue Errno::ENOENT
      return nil
    rescue => detail
